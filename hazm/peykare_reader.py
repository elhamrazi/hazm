...
def join_verb_parts(sentence):
	...
	if not hasattr(join_verb_parts, 'tokenizer'):
		join_verb_parts.tokenizer = word_tokenizer()
...
class peykare_reader():
	...
	def sents(self):
		...
		Examples:
			>>> peykare = peykare_reader(root='corpora/peykare')
			>>> next(peykare.sents())
			...
			>>> peykare = peykare_reader(root='corpora/peykare', joined_verb_parts=False, pos_map=None)
			>>> next(peykare.sents())
