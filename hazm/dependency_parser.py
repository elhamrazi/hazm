...
class dependency_parser(MaltParser):
	""" این کلاس شامل توابعی برای شناسایی وابستگی‌های دستوری است.

	این کلاس تمام توابع خود را از کلاس [MaltParser][hazm.dependency_parser.MaltParser] به ارث می‌برد.
	
	Examples:
		>>> from hazm import pos_tagger, lemmatizer
		>>> parser = dependency_parser(tagger=pos_tagger(model='resources/postagger.model'), lemmatizer=lemmatizer())
