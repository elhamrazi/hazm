...
class wikipedia_reader():
	...
	def __init__(self, fawiki_dump, n_jobs=2):	
		...
		self.wiki_extractor = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'wiki_extractor.py')
		self.n_jobs = n_jobs
	...
	def docs(self):
		...
		Examples:
			>>> wikipedia = wikipedia_reader('corpora/wikipedia.csv')
	...
	def texts(self):
		...
		‍[docs()][hazm.wikipedia_reader.wikipedia_reader.docs] و دریافت مقدار
