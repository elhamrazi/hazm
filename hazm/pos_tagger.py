...
from .sequence_tagger import sequence_tagger


class pos_tagger(sequence_tagger):
	"""این کلاس‌ها شامل توابعی برای برچسب‌گذاری توکن‌هاست. **میزان دقت برچسب‌زنی در نسخهٔ حاضر ۹۷.۱ درصد [^1] است.** این کلاس تمام توابع خود را از کلاس [sequence_tagger][hazm.sequence_tagger.sequence_tagger] به ارث می‌برد.
	[^1]: 
		این عدد با انتشار هر نسخه بروزرسانی می‌شود.
	"""
