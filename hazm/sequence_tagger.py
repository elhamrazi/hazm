...
class sequence_tagger(TaggerI):
	...
	def train(self, sentences):
		...
		Examples:
			>>> tagger = sequence_tagger(patterns=['*', 'u:word-%x[0,0]'])
	...
	def save_model(self, filename):
		"""مدل تهیه‌شده توسط تابع [train()][hazm.sequence_tagger.sequence_tagger.train] را ذخیره می‌کند. 
...
class IOBTagger(sequence_tagger):
	"""
	"""
