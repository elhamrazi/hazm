...
class informal_normalizer(normalizer):
	...
	def __init__(self, verb_file=informal_verbs, word_file=informal_words, seperation_flag=False, **kargs):
		...
		super(informal_normalizer, self).__init__(**kargs)
	...
	def normalized_word(self, word):
		...
		Examples:
			>>> normalizer = informal_normalizer()
			>>> normalizer.normalized_word('می‌رم')
			['می‌روم', 'می‌رم']

			>>> normalizer = informal_normalizer(seperation_flag=True)
	...
	def normalize(self, text):
		...
		text = super(informal_normalizer, self).normalize(text)
